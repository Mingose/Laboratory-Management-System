...
from resource.modle import Ui_Dialog
...
######继承modle显示界面控件类型
class showmodlepane(QDialog,Ui_Dialog):
...
def getdate(Recent_dbname, time):
    open_resentDB(Recent_dbname)
    modle = QSqlTableModel()
    initmodle(modle, time)
    view = createView("展示数据", modle)
    view.clicked.connect(findrow)
    dlg.mytableview.setModel(modle)
    # print(viem.__bass__)


def Modleshow(Recent_dbname,time):
    print("??????????????????????????")
    global view
    global dlg
    dlg = showmodlepane()
    view = dlg.mytableview
    getdate(Recent_dbname,time)
    dlg.move_bt.setEnabled(True)
...
if __name__ == '__main__':
    ...
    Recent_log ="yang2020-05-05-00-19-22"
    time = Recent_log

    Modleshow(Recent_dbname,time)
    sys.exit(app.exec_())